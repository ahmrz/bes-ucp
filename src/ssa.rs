...
pub fn ssa(
    ...
    for _ in 0..n_pop {
        ...
        let mut status: Vec<bool> = Vec::with_capacity(dim);

        for d in 0..dim {
            solution.value[d] = rng.gen::<f64>();
            status.push(transfer(solution.value[d], alpha.value[d], 0, max_iter, &mut rng));
        }
    ...
    for t in 0..max_iter {
        let c1: f64 = 2.0 * f64::exp(-(4.0 * t as f64 / max_iter as f64) * (4.0 * t as f64 / max_iter as f64));
        ...
        for p in 0..n_pop {
            ...
            if p < n_pop / 2 {
                for d in 0..dim {
                    ...
                    let c3: f64 = rng.gen::<f64>();
    
                    if c3 < 0.5 {
                        population[p].value[d] = alpha.value[d] + c1 * c2;
                    } else {
                        population[p].value[d] = alpha.value[d] - c1 * c2;
                    }
                ...
                for d in 0..dim {
                    population[p].value[d] = (population[p].value[d] + population[p - 1].value[d]) / 2.0;
                    status[d] = transfer(population[p].value[d], alpha.value[d], t, max_iter, &mut rng);
                }
            }
