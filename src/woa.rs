...
pub fn woa(
    ...
    // let mut alpha_cost: f64 = f64::INFINITY;
    let mut alpha: Solution = Solution::empty(dim, true);

    // let mut population: Vec<Vec<f64>> = Vec::with_capacity(n_search_agents);
    // let mut population_cost: Vec<f64> = Vec::with_capacity(n_search_agents);
    let mut population: Vec<Solution> = Vec::with_capacity(n_pop);

    /* Random solution initialization */
    for _ in 0..n_pop {
        // let mut sol: Vec<f64> = vec![0.0; dim];
        ...
        // population.push(sol);
        // population_cost.push(schedule.cost);

        if solution.cost < alpha.cost {
            alpha = solution.clone();
            // alpha_cost = population_cost[s];
            best = schedule;
        }
    ...
    for t in 0..max_iter {
        ...
        let a1: f64 = 2.0 - t as f64 * (2.0 / max_iter as f64);
        let a2: f64 = -1.0 + t as f64 * (-1.0 / max_iter as f64);
        ...
        for p in 0..n_pop {
            ...
            for d in 0..dim {
                if r3 < 0.5 {
                    if a3.abs() >= 1.0 {
                        // let rand_alpha_index: usize = f64::floor(n_search_agents as f64 * rng.gen::<f64>()) as usize;
                        let rand_alpha_index: usize = rng.gen_range(0..n_pop);
                        // let x_rand: Vec<f64> = population[rand_alpha_index].to_vec();
                        let d_x_rand: f64 = f64::abs(c2 * population[rand_alpha_index].value[d] - population[p].value[d]);
                        population[p].value[d] = population[rand_alpha_index].value[d] - a3 * d_x_rand;

                    } else {
                        let d_alpha: f64 = f64::abs(c2 * alpha.value[d] - population[p].value[d]);
                        population[p].value[d] = alpha.value[d] - a3 * d_alpha;
                    }

                } else {
                    let distance_2_alpha: f64 = f64::abs(alpha.value[d] - population[p].value[d]);
                    population[p].value[d] = distance_2_alpha * f64::exp(b1 * l1) * f64::cos(l1 * 2.0 * std::f64::consts::PI) + alpha.value[d];
                }
            ...
            // population[p] = sol;
            // population_cost[p] = schedule.cost;

            if population[p].cost < alpha.cost {
                alpha = population[p].clone();
                // alpha_cost = population_cost[p];
                best = schedule;
            }
        }
        // println!("Iteration {}, best: {}", t + 1, alpha.cost);
